...
import { generateCustomPurchaseOrderId, generateCustomRMAId, generateCustomProductId } from "../../../../customize/customizeId";
      ...
      <Page style={styles.page} size="A4" orientation="landscape">
        ...
        <View style={styles.table}> 
          <View style={styles.tableRow}>
            <Text style={styles.tableColHeader}>RMA ID</Text>
            ...
            <Text style={styles.tableColHeader}>Return Date</Text>
            ...
            <React.Fragment key={index}>
              {order.items.map((item, itemIndex) => (
                <View
                  ...
                  key={`${index}-${itemIndex}`}
                >
                  <Text style={styles.tableCol}>{generateCustomRMAId(order._id)}</Text>
                  <Text style={styles.tableCol}>{generateCustomPurchaseOrderId(order.po_id)}</Text>
                  <Text style={styles.tableCol}>{order.supplier_id?.company_name || "N/A"}</Text>
                  <Text style={styles.tableCol}>{formatDate(order.createdAt)}</Text>
                  <Text style={styles.tableCol}>{formatDate(order.return_date)}</Text>
                  <Text style={styles.tableCol}>{generateCustomProductId(item.product_id)}</Text>
                  <Text style={styles.tableCol}>{item.return_quantity}</Text>
                  <Text style={styles.tableCol}>{item.reason || "No description"}</Text>
                  <Text style={styles.tableCol}>{order.return_status}</Text>
