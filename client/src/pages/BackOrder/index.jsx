...
const BackOrder = () => {
  ...
  const handleConfirmDelete = async (id) => {
    try {
      ...
      localStorage.setItem("snackbarMessage", "Backorder delete successfully!");
      localStorage.setItem("showSnackbar", "true");
      window.location.reload();
      setMessage("Backorder delete successfully!")
        ...
        <Box sx={{ borderBottom: 1, borderColor: "divider", mb: 2 }}>
          ...
          <div className="table-responsive mt-3">
            <table className="table table-bordered table-striped v-align">
              ...
              {backorders.length > 0 ? (
                ...
                <tr key={backorder._id}>
                  ...
                  <td>{backorder.po_id.supplier.person_number}</td>
                  <td>{backorder.po_id.supplier.company_city}, {backorder.po_id.supplier.company_province}, {backorder.po_id.supplier.company_country},</td>
                  <td>{backorder.items.length}</td>
                  <td>{backorder.order_status}</td>
                  <td>
                    <div className="align-items-center d-flex justify-content-between">
                      {backorder.order_status === "Draft" ? (
                        <>
                          <Button variant="outlined" color="error" onClick={() => handleOpenDelete(backorder._id)}>
                            DELETE
                          </Button>
                          <Button variant="contained" color="primary" startIcon={<EditIcon />} onClick={() => handleEditBackorder(backorder)}>
                            EDIT
                          </Button>
                        </>
                      ) : backorder.order_status === "Approved" ? (
                        <Button variant="contained" color="secondary" onClick={() => handleOpenArchive(backorder._id)}>
                          ARCHIVE
                        </Button>
                      ) : null}
                    </div>
            ...
            <ConfirmationModal
              ...
              message="Are you sure you want to delete this backorder? This action cannot be undone."
              nameButton="Delete"
            />
            ...
            <ConfirmationModal
              ...
              message="Are you sure you want to archive this backorder?"
              nameButton="Archive"
            />
