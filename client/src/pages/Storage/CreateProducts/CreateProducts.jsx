...
const CreateProducts = () => {
  const [newProduct, setNewProduct] = useState({
    ...
    product_Profit_Margin: "", // Add this line
  });
    ...
    const fetchSuppliers = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/storageSupplier/"
        );
  ...
  const handleInputChange = (e) => {
    ...
    switch (name) {
      ...
      case "product_Profit_Margin":
        const profitMarginRegex = /^[0-9]+(\.[0-9]+)?$/; // Only numbers, optionally with a decimal point
  ...
  const handleCreateProducts = async () => {
    try {
      const response = await axios.post(
        "http://localhost:3000/api/storageProducts/bulk",
        productList
      );
